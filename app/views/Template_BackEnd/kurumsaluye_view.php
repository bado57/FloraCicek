<!-- İÇ SAYFA -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Kurumsal Üyeler 
        </h1>
    </section>
    <!-- Main content -->
    <section class="content">
        <!-- Main row -->
        <div class="row">
            <div class="col-xs-12">
                <!-- EKLEME DÜZENLEME FORMU -->
                <div class="box box-success collapsed-box">
                    <div class="box-header with-border">
                        <input name="kapaliacik" type="hidden" value="0" >
                            <input name="duzenleme" type="hidden" value="" >
                                <input name="duzenlemeID" type="hidden" value="" >
                                    <h3 class="box-title">Üye Bilgileri</h3>
                                    <div class="box-tools pull-right">
                                        <button id="formToggleBireyselUye" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-plus"></i></button>
                                    </div>
                                    </div><!-- /.box-header -->
                                    <div class="box-body">
                                            <div class="col-md-1 hidden-xs text-center" style="background-color: #61696D; padding: 30px 15px;">
                                                <i class="fa fa-briefcase" style="font-size: 80px; color: #e6e6e6;"></i>
                                            </div><!-- /.col -->
                                            <div class="col-md-3 col-xs-12">
                                                <table class="table table-responsive table-hover table-bordered table-condensed">
                                                    <thead class="bg-gray-light">
                                                        <tr>
                                                            <th colspan="2"><b>Üyelik Bilgileri</b></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr>
                                                            <td><b>Ad Soyad</b></td>
                                                            <td>Sn. <b>Ahmet Yılmaz</b></td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Durum</b></td>
                                                            <td>Kurumsal Üye</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Üyelik Tarihi</b></td>
                                                            <td>18/09/2015</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Kurum Adı</b></td>
                                                            <td>BRN Yatak</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>E-mail</b></td>
                                                            <td>info@deneme.com</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Telefon</b></td>
                                                            <td>0505 255 87 65</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Adres</b></td>
                                                            <td>Mevlana Mah. Turgut Özal Bulv. No:25 / 18 Talas / KAYSERİ</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Vergi Dairesi</b></td>
                                                            <td>ERCİYES</td>
                                                        </tr>
                                                        <tr>
                                                            <td><b>Vergi No</b></td>
                                                            <td>056855412</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                            
                                            <div class="col-md-8 col-xs-12">
                                                <table class="table table-responsive table-hover table-bordered table-condensed">
                                                    <thead class="bg-gray-light">
                                                        <tr>
                                                            <th colspan="4" class="text-danger"><i class="fa fa-warning" style="margin-right: 8px;"></i> 2 alışverişte toplam 225 TL tutarında satış yapıldı.</th>
                                                        </tr>
                                                        <tr>
                                                            <th><b>Sipariş No</b></th>
                                                            <th><b>Tarih</b></th>
                                                            <th><b>Toplam Tutar</b></th>
                                                            <th><b>İşlemler</b></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr>
                                                            <td>02684</td>
                                                            <td>12/15/2014</td>
                                                            <td>125 TL</td>
                                                            <td class="text-right"><a id="spDetay-1" class="btn btn-primary btn-sm" title="Detaylar"><i class="fa fa-eye"></i></a></td>
                                                        </tr>
                                                        <tr>
                                                            <td>23596</td>
                                                            <td>09/10/2015</td>
                                                            <td>100 TL</td>
                                                            <td class="text-right"><a id="spDetay-2" class="btn btn-primary btn-sm" title="Detaylar"><i class="fa fa-eye"></i></a></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                                <div class="form-group">
                                                    <input id="uyeDetayKapat" type="button" value="Kapat" class="btn btn-default pull-right" />
                                                </div>
                                            </div><!-- /.col -->
                                    </div><!-- /.box-body -->
                                    </div>
                <!-- LİSTE -->
                <div class="box box-info">
                    <div class="box-header with-border">
                        <h3 class="box-title">Kurumsal Üye Listesi</h3>
                    </div>
                    <div class="box-body">
                        <table id="uyeTable" class="table table-bordered table-hover">
                            <thead style="background:#e6e6e6;">
                                <tr>
                                    <th>Ad Soyad</th>
                                    <th class="hidden-xs">E-Posta</th>
                                    <th class="hidden-xs">Telefon</th>
                                    <th class="text-right">İşlemler</th>
                                </tr>
                            </thead>
                            <tbody class="">
                                <tr>
                                    <td>Ahmet Yılmaz</td>
                                    <td class="hidden-xs">a.yilmaz@gmail.com</td>
                                    <td class="hidden-xs">0505 265 75 85</td>
                                    <td class="text-right">
                                        <a id="uyeDetay" class="btn btn-primary btn-sm" title="Üye Detayları"><i class="fa fa-eye"></i></a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Hatice Kaya</td>
                                    <td class="hidden-xs">h.kaya@gmail.com</td>
                                    <td class="hidden-xs">0505 353 35 69</td>
                                    <td class="text-right">
                                        <a id="uyeDetay" class="btn btn-primary btn-sm" title="Üye Detayları"><i class="fa fa-eye"></i></a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div><!-- /.box-body -->
                </div><!-- /.box -->
            </div>
        </div><!-- /.row (main row) -->
    </section><!-- /.content -->
</div><!-- /İÇ SAYFA -->

<script type="text/javascript">
$(document).ready(function () {
    $("#uyeTable").DataTable({
        "paging": true,
        "lengthChange": true,
        "searching": true,
        "ordering": false,
        "info": false
    });
});
</script>